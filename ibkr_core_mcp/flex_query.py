...
# Page last updated 2025-08-18 (re-verified live 2026-07-10; supersedes a stale 2025-10-03
# note from the original 2026-06-30 Firecrawl scrape). All 21 codes present.
...
class FlexQueryClient:
    ...
    ## What Flex covers
    Activity Statements are account-level reports, not per-platform logs — they cover
    everything that happened in the account (positions, cash, fees, corporate actions,
    interest, dividends, trades, transactions), regardless of which client placed each
    trade (CP API, mobile app, TWS, web portal). The specific enumeration of origins is
    empirically confirmed for live CP-API trades by get_trades()'s "Origin coverage —
    verified live 2026-07-06" note in client.py; no single official page enumerates all
    four origins explicitly for Flex specifically.
