...
def test_execute_get_live_orders_shows_working_orders(toolkit):
    toolkit._client.get_live_orders.return_value = [
        {"orderId": 1, "ticker": "AAPL", "side": "BUY", "totalSize": 100, "price": 185.0, "status": "Submitted"},
    ]
    text, fig = toolkit.execute("get_live_orders", {})
    assert "AAPL" in text
...
def test_diagnose_orders_happy_path(toolkit):
    ...
    toolkit._client.get_orders_raw.return_value = {
        "orders": [
            {
                "orderId": 99,
                "ticker": "AAPL",
...
def test_get_order_status_happy_path(toolkit):
    ...
    toolkit._client.get_order_status.return_value = {"orderId": 42, "status": "Submitted", "filledQuantity": 0}
