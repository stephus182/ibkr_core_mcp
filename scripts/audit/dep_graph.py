...
def main() -> None:
    ...
    tree = ast.parse(src)
    cls = next(n for n in ast.walk(tree) if isinstance(n, ast.ClassDef) and n.name == "ClaudeToolkit")
    ...
    Path("docs/audits/audit-evidence/dep_graph.json").write_text(json.dumps(graph, indent=2))
