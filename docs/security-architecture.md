...
in `client.py`. If it writes an order, it is one of the six functions in `GATED_OWNERS` (the
five public writes plus `_resolve_one_reply`) or it does not
