...
| 2026-07-28 | **The vendor SDK's own default `strategy` is invalid against the vendor's own API.** `crawl4ai-cloud-sdk` 1.2.0 declares `scrape(strategy: str = "auto")`, but `/v1/scrape` accepts only `"browser"` or `"http"` and rejects `"auto"` with HTTP 422 (`literal_error`, `loc: ["body","strategy"]`). Every call left on the SDK default fails. We pass `"browser"` explicitly. Unit tests cannot catch this — a mocked crawler accepts any argument — so it surfaced only on the live suite, on the first run after adopting the SDK. |
...
| 2026-07-28 | **A consuming project can silently run stale ibkr_core_mcp code.** claudia_ui installs this package with `editable_mode=strict`, which resolves imports through a snapshotted symlink farm under `build/__editable__…/`. A module added after that install is **invisible** — `ibkr_core_mcp.crawl4ai_cloud` raised `ModuleNotFoundError` in ClaudIA while existing on disk here. Re-run the editable install in every consumer after adding a module, not just after adding a tool. |
| 2026-07-28 | **The `[scraper]` extra is not installed in claudia_ui**, so inside ClaudIA both fallback rungs were dark: no `crawl4ai` (rung 2) and no `crawl4ai-cloud-sdk` (rung 3). The cloud rung was enabled there by installing `crawl4ai-cloud-sdk`; **rung 2 remains unavailable in ClaudIA** until `crawl4ai` + `crawl4ai-setup` are installed, which is a heavy Playwright dependency and a deliberate open choice, not an oversight. |
| 2026-07-28 | The two Crawl4AI keys in this project (`ibkr_core_mcp/.env`, `claudia_ui/.env`) are **distinct** values — SHA-256 `0528d8cf…` vs `9d3b6a21…`, both 51 chars, both live, both reporting Free/50 credits. Compared by hash, not by prefix and length. One key per project is deliberate. **Settled 2026-07-28 by experiment: they SHARE ONE 50/day pool.** Spending exactly 1 credit on the claudia_ui key moved *both* ledgers 8 → 9. Distinct keys, one budget — so the daily allowance is 50 across both projects, not 100. |
