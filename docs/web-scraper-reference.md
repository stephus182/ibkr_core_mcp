...
| 2026-08-07 | **`firecrawl_search` had no quality signal at all, behind a doc line saying it had one.** `assess_quality`'s docstring named three callers; `grep` found two, both passing `metadata=None`, so its HTTP-status branch was unreachable in production — while `test_local_browser.py` covered that branch directly and stayed green. A passing test on dead code. `firecrawl_search` is the only tool that receives `metadata.statusCode` for free (verified against Firecrawl's published response schema, not assumed), so a 403 stub with a 44-byte body rendered exactly like a real result: the original §3.1 bug, still open on the one tool that could detect it. Now wired; flagged results are annotated, never dropped, and excluded from the read-these invitation. |
| 2026-08-07 | **`search_site` told the model "do not retry" after discovering zero pages.** `_is_no_information_plateau([])` returns `False` by design, so a dead sitemap, an unscored result set and a genuine no-match all returned `[]` and all printed *"its pages were read and scored — none of them is about that … do not retry"*. Two of those three are failures, and the instruction suppressed recovery from a transient 404. `search_site_detailed` now returns `{discovered, scored, searched}` and the handler branches three ways; the genuine no-match keeps its instruction and now states the count. |
| 2026-08-07 | **Live suite after the audit: `test_web_tools_live.py` 11/11 in 46 s, 0 skipped.** Run against real hosts with the repo's own standalone-dev `.env`. Covers both changes above plus the three SSRF private-host refusals. Unit suite 917 passed; `ruff check`, `ruff format --check` clean; `mypy` clean except one pre-existing error in a Flex test file, deliberately untouched (see `docs/plans/2026-08-07-flex-audit-handoff.md`). |
| 2026-09-17 | **Live suite after `SEC-R9` (the redirect walker now drops `Authorization` from the first cross-origin hop on): `test_web_tools_live.py` 12/12 and `test_crawl4ai_live.py` 1/1, 13 passed in 27 s, 0 skipped**, against real hosts with the repo's own standalone-dev `.env` — the real-Chromium redirect-to-loopback refusal included. The premise was measured before the change, not read off the docs: with two loopback echo servers on different ports, a header the page's own script set on its fetch arrived at the other origin whole through `route.fetch(url=…)`, and an explicit `headers=` override without it arrived with the same set minus that one. A first probe that set the header at context level (`extra_http_headers`) found it forwarded either way — Playwright applies context headers beneath any override — and was discarded as a confound. |
...
| 2026-08-11 | **IBKR moved the Web API path shape, not just the host.** `docs/web-api/web-api-v-1-0-documentation/…` is superseded by `docs/web-api/v1/…`, and the WebSocket segment `websockets/` is now `ws/`. Caught by `fetch_page` against `llms.txt`: 469 index entries, **zero** on the old prefix. All 72 unique repointed URLs were then live-fetched — 72/72 real — with a fabricated control URL in the same run returning `# Page Not Found`, proving the discriminator still discriminated at the end of the batch. 93 links repointed across living docs and source. |
| 2026-08-11 | **The `.md` trick only works on the `v1/` prefix, and the HTML page cannot be used to check existence.** Appending `.md` returns ~6,400 B on `v1/…` and a ~1,000 B `# Page Not Found` on the old prefix — the same size a fabricated URL returns. The plain HTML page is JS-rendered, so a real page and an invented one both come back **byte-identical and empty**. Verify via the `.md` variant with a fabricated control, never via the HTML page, a status code, or a byte count. |
| 2026-08-11 | **`www.interactivebrokers.com/docs/web-api/changelog` is real but absent from `llms.txt`.** Index membership is sufficient evidence of existence, not necessary — `firecrawl_search` returned the page as result 1 with live content ("Apr 14, 2026 … SMD topic will terminate after 1…") and a direct fetch returned 11,686 B. Whole-web search settled what the index could not, which is the one job Firecrawl still holds. |
| 2026-09-16 | **A search snapshot recorded a blocked page verbatim, with no marker.** `firecrawl_search` told the model "⚠ Not usable content (HTTP 403)" and passed the same raw list to `save_search`, which wrote `r["markdown"]` unchanged — so a 403 stub filed into `web_docs/searches/` read exactly like a real page (WEB-04). Fixed to the convention `crawl_site` already used: an all-unusable result set is **not archived** ("filing an error page into the archive would poison later research"), and a mixed set is archived with the unusable members marked. Verified against **real Drive**, not a mock: the file was downloaded and the marker asserted present, ahead of the payload, exactly once. Removing the annotation makes that live test fail. Live run: `test_web_tools_live.py` **12 passed**, plus `drive_live` / `dev_cache_live` / `web_scraper_live` / `crawl4ai_live` **9 passed** — 21 total, 0 failed. |
| 2026-08-11 | **WSJ is still blocked; a batch measurement said otherwise and was wrong.** A 61-URL sweep reported ~35 KB for `wsj.com`; re-running it alone returned `Blocked by anti-bot protection: DataDome captcha` with 1,265 B of captcha assets, despite a saved profile with a valid session. The 2026-07-30 finding stands unchanged. A number read out of a batch loop is not a measurement until it is reproduced on its own. |
| 2026-08-11 | **`docs.crawl4ai.com/advanced/` returns HTTP 403 to the automated browser** while its child pages serve normally (`advanced/session-management/` → 16,166 B). A section index can be blocked while everything under it is fine, so a single failed link does not condemn the section. Annotated in `external-docs-reference.md` rather than removed. |
| 2026-09-13 | **Live suite after the SSRF guard rewrite: 11 passed in 25 s.** The guard gained `inet_aton` literal parsing, the `100.64.0.0/10` range and an httpx request hook on `search_site`'s seeder (audit 2026-09-13 B7 and the same-day review). `tests/test_web_tools_live.py -m integration` ran against the real browser, real sitemaps and Firecrawl: all three tools returned real content, the anti-bot stub was still flagged, the error page still refused, and all three private-host refusals held. The rule that a scraper change is not done on a green unit suite applied here as anywhere: the unit table is DNS-free by design, so only this run shows the guard against live resolution. |
| 2026-08-11 | **61 non-IBKR external links swept.** All resolve except the two noted above; four zero-byte results are expected shapes, not breakage — two API bases (`api.firecrawl.dev/v1`, the Flex Web Service root) and a `pip install git+…@v1.0.0` spec, none of which is a browsable page. Judge a zero-byte result by what the URL *is* before calling it dead. |
