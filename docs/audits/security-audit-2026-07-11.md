...
**Status:** All 6 findings resolved (4 High, 2 Medium). Fixes: `4e38655..e587695` on branch `security-fixes-2026-07-11`, one commit per finding (plus 3 small follow-up commits from code-review loops) — see [`docs/plans/archive/security-orders/2026-07-11-security-fixes-plan.md`](../plans/archive/security-orders/2026-07-11-security-fixes-plan.md) for implementation detail on each. Every fix and its tests were independently reviewed for spec compliance and code quality before being accepted; two review rounds (H-2's regex, H-3's second stale doc reference) found real issues that were fixed forward in follow-up commits.
