...
| **`crawl4ai-cloud-sdk` on PyPI** (1.2.0, 2026-06-28 — the vendor's own thin client for the Cloud API. **Not used here**: `crawl4ai_cloud.py` is ~180 lines calling one endpoint with `requests`, already a dependency, and a hand-rolled client is what let the no-retry-on-429 rule and the credit accounting be written explicitly. Worth revisiting only if we ever need `/v1/site` job polling) | https://pypi.org/project/crawl4ai-cloud-sdk/ |
