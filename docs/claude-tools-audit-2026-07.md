...
D3 rejects Candidate 3 on its own evidence: with caching (decided, will exist) the layer-2 delta is 2.64% of the cached static prefix (Appendix A) and there are no measured wrong-tool selections (Appendix B pending) — caching does not leave a material problem, which is exactly the condition under which the design says *not* to build profiles.
...
| get_option_chain | **fix** | Description promises an options chain, but the handler delegates to `client.get_option_chain` → `GET /trsrv/secdef/chains`, which **does not exist** in IBKR's API (0 hits in the 2026-07-02 cpapi scrape; the documented flow is `/iserver/secdef/search` → `/iserver/secdef/strikes`). The call returns HTTP 404 every time — the tool is non-functional, and nothing in its Claude-facing text says so. | **description:** `Get the options chain for a symbol — expirations, strikes, and contract IDs. KNOWN ISSUE: the underlying endpoint (/trsrv/secdef/chains) is not part of IBKR's documented API and returns HTTP 404 on every call — this tool is currently non-functional pending reimplementation via the documented secdef/search → secdef/strikes flow.` Token +. | cpapi (no `/trsrv/secdef/chains`; documented `iserver/secdef/search`→`iserver/secdef/strikes`), 2026-07-02; corroborated by client.py:447 docstring |
