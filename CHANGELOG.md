...
### Changed
- **`search_contract` now puts the US listing first for STK, and its description no longer promises a currency it never returned** (2026-08-05). Measured live: `contracts[0]` for **IGV was the Mexican listing** (conid 325209548, MEXI), with US/BATS 12658199 second — while the tool description read *"Use this to discover conids before calling tools that require one"* and said nothing about country. The order path in the consuming app had already been changed to refuse a symbol-only order proposal, so placement was structurally safe; but the model was still being pointed at a tool whose first row is the wrong country, and correctness depended on it *noticing* `description: "MEXI"`. Rows now carry `_is_us` and sort US-first, because a bare ticker is a US ticker unless the user names another market — the same rule `_resolve_stock_conid` already applied, now applied here rather than restated.

  **The description was also wrong about the payload.** It advertised `exchange` and `currency`; the endpoint returns neither. There is no currency field at all, and the exchange appears only as `description` (a bare code) and inside `sections` — so a caller reading a currency off this response reads `None`, and currency is the one field that separates a US listing from its foreign twin. `client.search_contract`'s own docstring claimed the same two keys and is corrected. Both source URLs were the retired `cpapi-v1/#anchor` form, which returns HTTP 200 and silently drops the fragment; repointed.

  **Nothing is filtered and nothing is guessed.** All listings are still returned — hiding the foreign ones would make the genuinely ambiguous cases invisible, and IGV's third row is *I GRANDI VIAGGI SPA*, a different company on the same ticker. The sort is **stable**, so IBKR's own order survives within each group: that order is undocumented, and inventing a secondary key would swap one unfounded ordering for another. `_is_us` is **absent**, never `false`, when the check could not run — "not a US listing" and "could not check" are opposite claims. IND/BOND pass through untouched, since `/trsrv/stocks` is stocks-only.

  **One defect found by running it rather than testing it.** The first build required every returned row to have an `isUS` verdict, and silently did nothing for **AAPL and VOD** — 4 of 5 rows covered for both. The uncovered row is conid **2147483647** (INT_MAX): a *"Corporate Fixed Income"* aggregate with a null symbol and a BOND-only section, which the schema documents under bonds and which `/trsrv/stocks` rightly does not know. Non-stock rows are now excluded from the verdict by their `sections` list — not by the sentinel conid, which is one observed magic number where `sections` is the documented field — and kept, untagged, after the listings. IGV happened to have no such row, which is why the first live check looked like a pass.

  846 tests (9 over `search_contract`), both new assertions mutation-verified. Sources: [security-stocks-by-symbol](https://ibkrcampus.com/docs/web-api/v1/endpoints/contract/security-stocks-by-symbol.md) (`isUS` — *"States whether the contract is hosted in the United States or not"*), [search-contract-by-symbol](https://ibkrcampus.com/docs/web-api/v1/endpoints/contract/search-contract-by-symbol.md) (no currency field; ordering undocumented). Both read 2026-08-05.
